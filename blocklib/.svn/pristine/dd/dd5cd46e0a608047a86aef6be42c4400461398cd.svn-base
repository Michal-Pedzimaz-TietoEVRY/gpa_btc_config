...
%--------------------------------------------------------------------------
  ...
  methods
    ...
    function this = BandSplitterConfig()
        ...
        this.FirDown.SelectVariant("Simulink");
        ...
        this.FirUp.SelectVariant("Simulink");
    ...
    function val = get.Delay(this)
        ...
        % Add Delay for FirUp
        if this.FirUp.Variant == "WithFir"
            if this.FirUp.VariantObj.Fir.Variant == "SharcPlusAccel"
                val = val + this.FirUp.VariantObj.Fir.FrameSize*this.DecimationFactor;
            end
        elseif this.FirUp.Variant == "SharcPlusAccel"
            % If SharcPlusAccel Variant is supported in the future, will 
            % need to add additional delay equal to FrameSize*DecimationFactor
        end

        % Add Delay for FirDown
        if this.FirDown.Variant == "WithFir"
            if this.FirDown.VariantObj.Fir.Variant == "SharcPlusAccel"
                val = val + this.FirDown.VariantObj.Fir.FrameSize;
            end
        elseif this.FirDown.Variant == "SharcPlusAccel"
            % If SharcPlusAccel Variant is supported in the future, will 
            % need to add additional delay equal to FrameSize*DecimationFactor
        end
    end
