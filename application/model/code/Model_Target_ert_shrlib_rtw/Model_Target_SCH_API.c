...
const char* talariaVersion1 = "V2200BB";
...
Model_Target_RTC_REQ_RSP_type SchDispathTable1[] =
{
    {
...
void SchReqRsp_GetTalariaVersion1(void* pREQBuf, void* pRSPBuf)
{
    ...
    for (int i=0; i < sizeof(payloadRSP->version_string); i++)
    {
...
static void SharedGetHash(void* pREQBuf, void* pRSPBuf, uint32_t* nameArray,
    uint32_t* hashArray, uint32_t numElements)
{
    GetCompatHash_REQ_T* payloadREQ = (GetCompatHash_REQ_T*)pREQBuf;
    uint32_t callerHandle = payloadREQ->caller_handle;
    ...
    GetCompatHash_RSP_T* payloadRSP = (GetCompatHash_RSP_T*)pRSPBuf;
    payloadRSP->caller_handle = callerHandle;
    payloadRSP->status_code = statusCode;
...
int Model_Target_UseDispatchTable(void* pReqBuf, uint32_t reqBufSize,
    ...
    Model_Target_RTC_REQ_RSP_type* theTable, uint32_t tableSize)
{
    ...
    if (rtcIdx < tableSize)
    {
        ...
        Model_Target_RTC_REQ_RSP_type* pTableEntry = &theTable[rtcIdx];
    ...
    void* pRspBuf, uint32_t rspBufSize)
{
    ...
    Model_Target_RTC_REQ_RSP_type* theTable = SchDispathTable1;
