...
methods
%--------------------------------------------------------------------------
function this = ExtractModelBTC(modelName,filename,bootStrap)
    if (nargin < 3)
        bootStrap = false;
    end
...
function DumpClassdef(this,componentName,parentElem,obj,elementType)
    ...
    for propIdx = 1:numProps
        ...
        [canGet,canSet,isUserBtcClass,requiredClassObj] = BtcBaseClass.GetPropertyStats(aProp);
        isBuiltIn = (isUserBtcClass == false);
        propClass = requiredClassObj;

        if (false == canGet)
            % If cannot even get it, certainly not BTC
            continue;
        end
        if (false == canSet && isBuiltIn)
            % If you cannot set it and it's not a user class: not BTC
            continue;
        end

        if (false == isBuiltIn && false == isempty(propClass) && ...
                false == propClass.Enumeration)
