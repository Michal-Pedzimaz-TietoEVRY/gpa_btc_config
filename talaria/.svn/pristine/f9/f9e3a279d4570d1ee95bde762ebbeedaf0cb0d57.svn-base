...
methods(Static, Hidden)
    ...
    %----------------------------------------------------------------------
    function [canGet,canSet,isUserBtcClass,requiredClassObj] = GetPropertyStats(aMetaPropertyObject)
        ...
        % Note: isUserBtcClass returns false unless the property has a
        % specified class is user-defined, and the property is not
        % dependent or hidden.
        % Usage:
        %    [canGet,canSet,isUserBtcClass,requiredClassObj] = BtcBaseClass.GetPropertyStats(aMetaPropertyObject)
        ...
        if ( nargout > 2)
            % Assume the class is not user-defined BTC
            isUserBtcClass = false;
            if (false==isempty(aProp.Validation) && false==isempty(aProp.Validation.Class))
                requiredClassObj = aProp.Validation.Class;
                isBuiltIn = ismember(requiredClassObj.Name,{'double' 'single' 'int8' ...
                    'uint8' 'int16' 'uint16' 'int32' 'uint32' 'boolean' 'char' 'string'});
                % For "BTC" purposes, anything that's an inherent type, or
                % a hidden or dependent property is not considered
                isEnum = requiredClassObj.Enumeration;
                isUserBtcClass = (false == (isBuiltIn || aProp.Hidden || aProp.Dependent || isEnum));
            else
                requiredClassObj = [];
            end
        end
