classdef BtcTreeView < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function TopPanelSizeChanged(app,event)
            ...
            app.ClearButton.Position = [newWidth-45, editPos(2), 40, editPos(4)];
        ...
        function ClearButtonPressed(app)
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.ClearButton = uibutton(app.TopPanel);
            app.ClearButton.Text = 'Reset';
            app.ClearButton.ButtonPushedFcn = createCallbackFcn(app,@ClearButtonPressed);
    ...
    methods (Access = private)
        function LoadObject(app,objectVarName,nodeObject)
            ...
            btcObj = evalin('base',objectVarName);
            btcMetaClass = meta.class.fromName(class(btcObj));
            if isa(btcObj,'BtcLockAndDefaultClass')
                isBtc = true;
            else
                isBtc = false;
            end
            for i=1:numel(btcMetaClass.PropertyList)
                ...
                if ((false == canSet) && (false == isUserBtcClass))
                ...
                propValue = btcObj.(aProp.Name);
                propIsBtc = isa(propValue,'BtcLockAndDefaultClass');
                if (isBtc)
                    propIsLocked = btcObj.IsLockedOrDefaulted(aProp.Name);
                else
                    propIsLocked = false;
                end
                ...
                if (app.UseStyles)
                    ...
                    elseif (isobject(propValue) && isUserBtcClass)
                        addStyle(app.Tree,app.StyleIconObj,"node",newNode);
                    else
        ...
        function UpdateTextArea(app,theNode)
            ...
            if isa(rootObject,'BtcLockAndDefaultClass') && theNode ~= rootNode
                ...
                if (false == isempty(theEntry))
                    ...
                    [hasLock,usingDefault] = propOwner.IsLockedOrDefaulted(propName);                    
