classdef BTConfig
    ...
    function [hasConfig, configVarName] = HasModelLevelConfig(aBlockURL)
        ...
        rootName = ModelRootName(modelName);
        configVarName = [rootName BTConfig.ModelVarNameEnd];
        % Pretend to add code - if code is added, it means it wasn't there.
        assertCode = sprintf(BTConfig.ModelAssertLine,configVarName);
        currentCode = get_param(modelName,'InitFcn');
        [~, wasAdded] = BTConfig.AddCode(currentCode,assertCode,BTConfig.AddToStart);
        if (wasAdded)
            % The model-level BTC code was not there.
            hasConfig = false;
        else
            % The model-level BTC was already there. This is setup for
    ...
    function AddChildBlockConfig(theChildBlockHandle)
        ...
        if (isempty(parentMask) || (false == parentMask.isMaskWithDialog))
            ...
            if (false == BTConfig.HasModelLevelConfig(theChildBlockHandle))
    ...
    function AddAllTuneVars(blockHandle, useSeparateTuneConfigClass)
        ...
        % Main class mode adds "Default" prefixed properties to the same 
        ...
        for i=1:numel(allTuneBlocks)
            ...
            if (isempty(featurePath))
                ...
                internalPath = extractAfter(featurePath,blockFeaturePath);
            end
            % If adding to the main Config class, prefix with "Default"
            if (false == useSeparateTuneConfigClass)
                internalPath = ['Default' internalPath];
            end
    ...
    function PrintDiscoveredBtc(aBlock)
        ...
        if (hasModelBtc)
            mulliganLink = sprintf(['<a href="matlab:BTConfig.GetModelSettings;' ...
    ...
    function addedIt = AddClassdefProperty(className,newPropertyName,componentConfigClass,defaultValueStr,propertiesLine)
        ...
        % Note that the option 'dotexceptnewline' is handy, but 
        % experimentation was easier using the  the expression [^\n] for
        % dot-except-newline characters.
        % First decorate the "properties" header line with whitespace
        propertiesExp = BTConfig.CreateRegexCodeSearch(propertiesLine);
        % Search with positive lookbehind for "properties" line plus misc,
        % and with positive lookahead for an "end" line
        insertSearchExp = ['(?<=^[^\n%]*' propertiesExp '.*).(?=^\s*end)'];
        insertionIndexes = regexp(theDoc.Text,insertSearchExp,'lineanchors');
        if (isempty(insertionIndexes))
            % Try to insert
            BTConfig.InsertPropertiesBlock(theDoc,className,propertiesLine);
            % Try to find again
            insertionIndexes = regexp(theDoc.Text,insertSearchExp,'lineanchors');
            if (isempty(insertionIndexes))
                error(['Unable to find the "%s" section in class [%s]. Unable to proceed.\n' ...
                    'Add this classdef section yourself and try again.'], ...
                    propertiesLine,MaskUtil.GetFileEditLink(className));
            end
            % else
        end
        % Insert at the first place identified - this is the first end
        % found after the "properties" line:
        insertionIndex = insertionIndexes(1);
        lineNumber = numel(regexp(theDoc.Text(1:insertionIndex),'\n'))+1;
    ...
    %----------------------------------------------------------------------
    function InsertPropertiesBlock(theDoc,className,propertiesLine)
        ...
        newCode = sprintf('  %s,GetObservable,SetObservable)\n  end\n', ...
            propertiesLine);
        theDoc.insertTextAtPositionInLine(newCode,lineNumber,1);
        hereLink = sprintf('<a href="matlab:edit(''%s'');goToLine(matlab.desktop.editor.getActive,%d);">HERE</a>', ...
            className,lineNumber);
